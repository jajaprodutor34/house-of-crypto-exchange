...
export default function App() {
  ...
  // NOVOS STATES DO SWAP
  const [swapFrom, setSwapFrom] = useState("MATIC");
  const [swapTo, setSwapTo] = useState("MGC");
  ...
  useEffect(() => {
    ...
    const updated = pools.map((p) => {
      ...
      if (isException(addrOrSym))
        return { ...p, visible: true };

      if (p.liquidityUSD >= MIN_VISIBLE_USD)
        return { ...p, visible: true };
  ...
  const connectWallet = async () => {
    ...
    try {
      ...
      const chainIdHex = await provider.send(
        "eth_chainId",
        []
      );
  ...
  const createPool = async ({
    ...
    if (!name || !symbol || !address || !pair)
      return alert("Fill all info.");
    ...
    if (
      initialLiquidityUSD < MIN_VISIBLE_USD &&
      !isException(symbol)
    ) {
      alert(
  ...
  const addLiquidity = async (
    ...
    if (
      ...
      return alert("Invalid pool/amount.");
    ...
    if (idx === -1)
      return alert("Pool not found.");

    const clone = [...pools];

    clone[idx].liquidityUSD =
      (clone[idx].liquidityUSD || 0) +
      amountUSD;

    refreshVisibility(clone);

    alert(
      `Added $${amountUSD} liquidity to ${clone[
        idx
      ].pairs.join(", ")}. Current: $${
        clone[idx].liquidityUSD
      }`
  ...
  const handleWithdraw = () => {
    ...
    if (amount > available)
      return alert("Insufficient liquidity.");

    clone[idx].liquidityUSD = Math.max(
      0,
      available - amount
    );
  ...
  const poolVisibilityText = (p) => {
    if (
      isException(p.address) ||
      isException(p.symbol)
    )
      return "Visible (exception)";

    if (p.liquidityUSD >= MIN_VISIBLE_USD)
      return `Visible (liquidity $${p.liquidityUSD})`;
    ...
    >
      ...
      {page === 1 && (
        <div className="page-iniciante">
          <h1>
            Welcome to House of Crypto Exchanges
          </h1>

          <p>
            Trade tokens easily and quickly.
          </p>
          ...
          <div className="actions">
            ...
            <button
              onClick={() => {
                const address = prompt(
                  "Token address to add liquidity (0x...):"
                );
